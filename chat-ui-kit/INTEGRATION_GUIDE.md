...
5. **`generatePage`** command: Opens full-viewport immersive page
   - Takes over the ENTIRE screen (no side panel, no gallery) with a floating close button
   - Supports `<style>` tags, `@keyframes`, animations, background images, scroll effects
   - CSS variables from the parent page are auto-injected so the page matches the site theme
   - The visitor experiences it like a standalone website page, then closes with the X button

The `showGallery()` and `showLanding()` callbacks control transitions between your gallery view and landing page. ChatUI handles the split-screen overlay, canvas, and immersive page transitions internally.
...
// Test generatePage (immersive animated page)
ChatUI.executeCommand({
  action: 'generatePage',
  title: 'Immersive Test',
  html: '<style>@keyframes fadeUp{from{opacity:0;transform:translateY(40px)}to{opacity:1;transform:translateY(0)}}.hero{min-height:100vh;display:flex;align-items:center;justify-content:center;background:var(--color-bg)}.hero h1{font-family:var(--font-serif);font-size:3rem;color:#fff;animation:fadeUp 1s ease-out}</style><div class="hero"><h1>Immersive Page Test</h1></div>'
});
```
