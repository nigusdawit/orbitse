...
- **Tables**:
  ...
  - `presentations` — **Phase A: Agentic Skills.** Slide decks the AI launches on the visitor's screen with voice narration. Has slug (unique), title, description (one short sentence the AI uses to decide when to offer the deck), cover_image_url, source ('admin' for admin-typed decks, also used for imported decks), auto_play (boolean — when true the AI launches without asking first), enabled, created_at, updated_at. **Decks can also be created by upload via `POST /admin/api/presentations/import`**: PDF (each page rendered to a JPG image stored under `/uploads/`, page text becomes narration_text), PPTX (text + speaker notes only — visual fidelity requires saving as PDF first; uses `python-pptx`), or a set of JPG/PNG/WEBP images (one slide each, ordered by filename). PDF rendering uses PyMuPDF (`fitz`) at 2× matrix capped at 1600px wide / 85% JPEG quality. The "Import deck" button on the Presentations admin tab triggers this flow and opens the resulting deck in the editor for review.
  ...
  - `agent_skills` — Registry of every skill (tool) the chat AI can call. Auto-synced on startup from `SKILL_METADATA` in app.py. Has name (unique), display_name, description, category (lookup, presentation, ...), enabled (admin toggle that drives `get_active_chat_tools`), builtin (true for code-defined skills), config_json (per-skill overrides), created_at, updated_at. **Admin CRUD on the AI Skills tab**: builtins can be enabled/disabled and have their display_name, description, and category rewritten (the startup sync uses `COALESCE(NULLIF(...))` on all three fields so admin edits are never clobbered — only the `builtin=true` flag is hard-set from code); custom skills (builtin=false) can be added via "+ Add custom skill" — they're no-arg tools whose `config_json.response_text` is returned verbatim to the AI when invoked, and they can be edited or deleted. Custom-skill names are validated against `^[a-z][a-z0-9_]{1,59}$` and rejected if they collide with a builtin. The category field in the editor is a `<datalist>`-backed input pre-populated from categories already in use. Endpoints: `POST /admin/api/skills`, `PUT /admin/api/skills/<id>` (extended), `DELETE /admin/api/skills/<id>` (custom only — builtins refuse with a 400 explaining they'd resync on next restart). **Live-managed prompt**: every chat turn appends a `TOOLBOX` section to the system prompt built from `SELECT name, display_name, description, category FROM agent_skills WHERE enabled = true` (grouped by category, builtins first), so admin edits to a builtin's description and every custom skill's description ACTUALLY drive the model's tool-selection behavior — not just the cosmetic admin label.
...
- **Database tables**: `automations`, `automation_runs`, `automation_versions`, `automation_settings`
