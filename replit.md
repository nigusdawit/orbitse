...
- **Proactive AI Voice Agent** — three independent toggleable features: (1) UTM-targeted **welcome voice intros** (pre-generated MP3s cached on disk, matched against utm_source/medium/campaign/referrer with priority tie-break, shown via "tap to listen" glass card or autoplay); (2) **visitor voice input** via browser-native Web Speech API (free, mic button injected into all 3 chat input rows); (3) **AI voice replies** via OpenAI TTS (audio cached by content hash so identical replies are free; per-IP daily char cap of 30k prevents abuse). Tables: `voice_settings` (singleton), `voice_intros`, `voice_usage_log`. Admin "Voice Agent" tab manages all three. Frontend logic in standalone `public/voice.js`. Usage stats panel surfaces TTS character totals (today/week/month) for client billing
