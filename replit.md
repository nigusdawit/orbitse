...
- **Client**: `_mcp_post`/`_mcp_initialize`/`_mcp_list_tools`/`_mcp_call_tool` in `app.py` speak JSON-RPC 2.0 over Streamable HTTP, accept both `application/json` and the first `text/event-stream` data event, and reuse the existing `_webhook_url_safe` validator + DNS pin patch so the SSRF block (localhost / private IPs / link-local) applies to MCP URLs too. SSE transport and OAuth auth are scaffolded but return clear "not enabled yet" errors at the client and REST layers.
...
- **V2 scaffolding**: `MCP_CONNECTOR_BLUEPRINTS` registry (currently empty) is reserved for OAuth-flowed connectors (Linear, GitHub, etc.). When the registry is empty, every code path that sees `auth_type='oauth'` returns a clear "OAuth connectors are not enabled yet" 400 — REST API, admin chat propose tools, and the MCP client itself. The `oauth_state JSONB` column on `mcp_servers` and the `connector-blueprints` listing endpoint are already wired so adding a real blueprint in the future is a strictly additive change.
