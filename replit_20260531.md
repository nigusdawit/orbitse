...
- **Admin Chat Knowledge Base (RAG)** — admin chat sidebar gets a **📚 KB** button that opens a Knowledge Base manager: admin uploads PDF/DOCX/PPTX/CSV/TXT files (25 MB cap, ext-allowlisted), the server extracts text (`pypdf`, `python-docx`, `python-pptx`, csv stdlib), chunks it into ~800-token sliding windows with ~100-token overlap (word-based approx — no tiktoken dependency), embeds via OpenAI `text-embedding-3-small` (1536d, batched 100 at a time) and stores per-chunk vectors in **pgvector** with an HNSW cosine index. On every admin-chat turn, if the per-session **Use Knowledge Base** toggle (in the Skills modal) is on, the user message is embedded and the top-K=6 nearest chunks across all of that tenant's documents are auto-injected as a system message with `[source: file p.N]` markers; the AI is instructed to echo those markers verbatim and the chat renderer linkifies each one into a clickable pill that opens a Citation Viewer modal showing the actual chunk text (resolved server-side via `/admin/api/kb/preview`). A `kb_retrieval` SSE event also surfaces the matched chunks as a transparent "📚 Searched Knowledge Base: …" badge above the agent bubble. The AI can also call **`lookup_knowledge_base`** as an OpenAI function-call tool for explicit drill-down searches. All embedding calls (auto-retrieve + tool-call + bulk upload) record `rag_embed` rows in `api_cost_events` against the seeded `text-embedding-3-small` model_prices row ($0.02 / 1M tokens), so spend shows up in the Cost tab beside chat/voice/SMS. A scheduler tick fires every 6 hours and re-embeds any document whose stored mtime changed since last index (useful when an admin replaces a file in place). Tables: `rag_documents` (one row per file, tracks status/page_count/chunk_count/storage_key/source_mtime), `rag_chunks` (one row per ~800-tok chunk, vector(1536) embedding, ON DELETE CASCADE from documents). New column: `admin_chat_sessions.use_kb BOOLEAN DEFAULT TRUE`. New endpoints: `/admin/api/kb/{list,upload,preview}`, `/admin/api/kb/<id>`, `/admin/api/kb/<id>/reindex`, `/admin/api/kb/chunk/<id>` (all `@admin_required`). No LangChain — pure pgvector + the existing `openai_client`.
...
## Admin UI for Snapshot/Clone (Tier 10 — May 2026)

Tenth slice of the onboarding work. Closes the loop opened by Tiers 5 + 6 (which shipped the snapshot engine as a CLI + admin-side extension): the same machinery is now reachable from the admin dashboard for operators who don't have shell access on the deployed install. Tier 5/6 originally noted "the wizard JSON-upload tile [is the] intended future enhancement (Tier 5+)" — Tier 10 fills that gap on the export side with a Clone & Snapshot tab + a VELO command, both delegating to `scripts.snapshot.build_snapshot` so the CLI, the dashboard, and the VELO surface can never drift on what gets exported, what's redacted, or how the JSON is shaped. Zero schema changes — pure HTTP/UI wrapper around existing read-only export code.

- **One engine, three surfaces**: `scripts/snapshot.py` gained one new public function — `summarize(snapshot)` — that reduces a built snapshot dict to row counts (`settings_keys[]`, `features.{mode,plan,count}`, `faqs_count`, `content_counts{}`, `admin_record_counts{}`, `exported_at`). Cheap (no extra DB hits) and used by both the dashboard preview pane and the VELO `summary_only` mode. Detects nested children (currently only `dashboards.widgets`, but the structure generalises across future Tier 6 children) by scanning each row dict for list-typed values and summing per child key.
- **HTTP routes (`app.py`, after the AI-cache bump-version route)**:
  - `GET /admin/api/onboarding/snapshot` — full JSON payload. With `?download=1` returns it as a `Content-Disposition: attachment` named `snapshot-<site-slug>-<YYYYMMDD>.json` (slug derived from `site_settings.site_name` so multi-install operators can tell their downloads apart at a glance). `Cache-Control: no-store` on the download path so intermediaries can't cache install-private config.
  - `GET /admin/api/onboarding/snapshot/summary` — counters only, for the dashboard preview. Echoes any `errors` from `build_snapshot` as a `warnings[]` array so the operator can see when a section read partially.
  - Both use the new `_parse_snapshot_args()` helper to translate the dashboard's checkbox query-string into the argparse-Namespace shape `build_snapshot(args)` expects. UI defaults differ from CLI defaults — UI biases toward the agency-clone use case (admin records ON, content OFF, secrets OFF) since the operator can see and toggle each before downloading; CLI biases toward minimal stdout-pipe output. Strict bool parsing on every flag (`"0"` / `"false"` / `"no"` / `""` all → `False`) so a typo in a query string can never accidentally export secrets.
  - Both are GET-only so the Tier 7 CSRF middleware is a no-op (CSRF only validates POST/PUT/PATCH/DELETE). `@admin_required` (session cookie) is the auth boundary.
  - `admin_user` is intentionally NOT included in the dashboard's default download — receivers should add their own admin email by editing the JSON before applying. The CLI keeps its `--include-admin-user` flag for the agency-master case where it's wanted.
  - The download path can't go through `jsonify()` — Flask's encoder doesn't handle `Decimal` (e.g. `model_prices.input_price_per_million_tokens`), `datetime`, `UUID`, or `bytes`. Both endpoints route through `json.dumps(default=_snap._json_default)` so every column type Postgres returns serialises cleanly.
- **VELO command (`velo_handlers.py`, end of file)**: new `export_install_snapshot` (`requires_confirmation=False` because it's read-only). Same param surface as the HTTP route plus `summary_only` for previewing-before-pulling-100KB. Lets a VELO master agent fetch a template from one configured install and POST it to `bootstrap_install` on another — full clone with no human in the loop, no browser file transfer. Capability count: 32 → 33.
- **Admin dashboard tab (`templates/admin/dashboard.html`)**: new "Clone & Snapshot" sidebar button (after Secrets, before Saved Pages — natural neighbourhood for operator-tools), backed by a tab panel with three checkboxes (admin records / content rows / sensitive cols), three buttons (Download JSON / Copy to clipboard / Refresh preview), an inline preview pane that lists settings keys + feature counts + FAQ count + content-row counts + admin-record counts (with nested children like `dashboards: rows: 1, widgets: 1`), and a four-step "How to apply this on a fresh install" instructions block. Preview re-fetches on every checkbox change so the operator sees exactly what's in the file before downloading.
- **Hardening from code review (post-build)**: three follow-ups landed before sign-off — (1) `_json_default` in `scripts/snapshot.py` now also handles `uuid.UUID` (Postgres uuid columns become `uuid.UUID` via psycopg2 — no current admin table uses them but several content tables do, e.g. `site_visitors.id`, so enabling content export was a latent crash); (2) `Cache-Control: no-store` is now set on the inline (copy-to-clipboard) path AND the summary endpoint, not only the download — they all carry install-private metadata (settings keys, per-table counts, full business config) and a shared cache replaying any of them across operators is the same blast radius; (3) the VELO `export_install_snapshot` adapter swapped `bool(p.get(...))` for a strict `_strict_bool` matching `_parse_snapshot_args` — the naïve `bool("false") == True` quirk meant a master agent passing `include_admin_secrets="false"` would have silently exported secrets. `_strict_bool` accepts real booleans + numeric truthiness, matches strings against `{1,true,yes,on}`, and falls back to the per-flag default for anything else (so a typo can never enable a sensitive flag). Eight unit-style assertions exercise the truth table end-to-end (`"false"`, `"0"`, `"no"`, omitted → False; `"true"`, `True`, `"on"`, `"yes"` → True) — all pass.
- **UX clarification**: the dashboard "Content rows" checkbox label initially implied a broad export but actually triggers `include_content=""` which is the engine's sentinel for the default subset (services + team only). Re-worded to "exports the default subset (services + team only)" with a hint pointing to the CLI's `--include-all-content` for the wider set, so what the operator reads on the screen matches what lands in the JSON.
- **Verification**: full request transcript captured against the live dev DB. `GET /admin/api/onboarding/snapshot/summary` returns the 6-section settings list (including `agent_provider_settings` from Tier 6's admin-singletons), `features.set` with 16 enabled flags, the 7 admin record tables with correct row counts (`agent_skills: 64`, `automations: 1` rows + `1` action_step, `dashboards: 1` row + `1` widget, `mcp_servers: 2`, `messaging_templates: 1`, `model_prices: 10`, `custom_sql_skills: 1`), and zero warnings on the default config. `GET /admin/api/onboarding/snapshot?download=1` returns HTTP 200, 104 KB payload with the right `Content-Disposition: attachment; filename="snapshot-theta-sync-20260501.json"` header and `Cache-Control: no-store`. The inline (no-download) path and summary endpoint both also carry `Cache-Control: no-store` after the hardening. Toggling `include_admin_secrets=1` adds the `mcp_servers.auth_credential` warnings as expected; toggling it off (default) keeps them redacted. `admin_user` is NOT in the downloaded JSON (default UI semantics), `$schema_version: 1` and `$description` carry through. JSON parses cleanly end-to-end.
...
## CDN for /uploads/ (Optimization #5 — April 2026)

Fifth item from the perf list. Tier 9 added a pluggable storage backend (`storage.py`) that already exposes `UPLOADS_PUBLIC_BASE_URL` for emitting bucket-direct URLs, and Optimization #4 added responsive WebP variants. But until this optimization, flipping `UPLOADS_PUBLIC_BASE_URL` did **nothing observable** — every image byte still flowed through Flask, because nothing in the request-emission path (API responses, render helpers, serve routes) read the env var. Optimization #5 wires it through end-to-end so an operator can put a CDN (CloudFront / Cloudflare / Bunny / Fastly / anything) in front of `/uploads/` by setting one env var, and stop serving image bytes from origin entirely.

- **Two complementary layers, both gated on `UPLOADS_PUBLIC_BASE_URL`**: (1) **Origin redirect (safety net)** — `serve_upload` (`app.py:19867`) and `serve_voice_file` (`app.py:22395`) return `301` to `<base>/uploads/<filename>` when the env var is set. Catches every byte of traffic that didn't come through the frontend's CDN-aware helper: cached HTML in browser histories, externally-shared image links, RSS readers fetching blog covers, social-media unfurlers, search-engine image crawlers, the lot. The 301 itself is cached (`Cache-Control: public, max-age=86400`) so subsequent requests skip the origin entirely. (2) **Frontend rewrite (perf optimization)** — `/api/page-bundle` exposes the base in a new `uploads_public_base_url` field (`app.py:6143`); `imgSrcset` and `imgAttrs` in `public/script.js` (lines ~5611 and ~5639) prepend it so freshly-rendered `<img>` tags point straight at the CDN with zero origin round-trip. The Python equivalent — `image_optimize.srcset_for` (`image_optimize.py:323`) — gained a `base_url=` param that defaults to reading the same env var, so any server-rendered srcset emission stays consistent.
- **Cache-Control headers (huge win on its own, even WITHOUT a CDN)**: filenames in `/uploads/` are content-hashed by design — originals are 16-hex stems from `_unique_upload_name` (`app.py:19751`), variants are deterministic `<stem>-<width>.webp` from `image_optimize._variant_filename`, voice cache is SHA-1 from `_voice_cache_filename`. A given URL truly never changes its bytes. So `serve_upload` now sets `public, max-age=86400` (1 day) on originals, `public, max-age=2592000, immutable` (30 days) on variants, and `serve_voice_file` sets the 1-day flavor too. Repeat visitors stop re-fetching every image on every page load — the perf gain shows up immediately even on a deployment that never sets `UPLOADS_PUBLIC_BASE_URL`. The 86400/2592000 split exists because variant URLs are mathematically derived from a hash and a width (`abc123-800.webp` always means the 800w of `abc123.*`), whereas originals — though hash-named — could in principle be re-uploaded with the same filename by an admin one day; we keep the door open. The `immutable` directive on variants tells modern browsers to skip even the 304-revalidation round-trip, the strongest cache hint available.
- **Origin-pull bypass (loop prevention)**: an operator can configure their CDN's origin to point either at the bucket directly OR back at this Flask app (the latter for shops not using S3-compatible storage). In the second case, the CDN's own pulls through `/uploads/` would be redirected back to the CDN — infinite loop. Solution: the CDN sets a custom request header `X-CDN-Origin-Pull: 1` on origin requests, and `_is_origin_pull_request()` (`app.py:19861`) makes serve_upload skip the redirect on that header. Bunny CDN: "Custom Headers" tab. Cloudflare: a Page Rule or a Worker. CloudFront: an Origin Custom Header. The header is checked strictly (`== "1"`) so a garbage value doesn't accidentally bypass.
- **Hardened bypass (optional secret)**: the simple `X-CDN-Origin-Pull: 1` setup is fully spoofable — any client can send that header and force origin to serve bytes (a performance-degradation / DoS-amplification attack). For deployments that care, setting `UPLOADS_CDN_PULL_SECRET=<random-token>` flips the bypass into strict mode: the header value must equal the secret to skip the redirect. Comparison uses `secrets.compare_digest` (constant-time) to avoid timing oracles. When the env var is unset (default), behavior is the simple `: 1` mode for backward compatibility — operators opt in to the harder mode by setting the secret. Pin tested in `TestUploadsCDNPullSecret`.
- **Self-loop guard (foot-gun protection)**: if an operator misconfigures `UPLOADS_PUBLIC_BASE_URL=https://my-flask-app.example.com` (i.e. points it at the SAME host as the live app), every `/uploads/` request would 301 back to itself until the browser hits its redirect-chain limit (~20) and gives up — every page load broken site-wide. `_base_points_at_self()` (`app.py:19883`) detects this by comparing the parsed netloc of the base against `request.host` (case-insensitive); on match, the redirect is skipped and bytes flow normally — degraded perf but working site, instead of an unrecoverable loop. The CDN's normal origin-pull case is handled separately by the bypass header so this guard fires only on real misconfiguration. Pin tested in `TestSelfLoopGuard`.
- **Variant pre-generation BEFORE redirect**: when the redirect path triggers for a variant URL, the on-demand variant generator runs FIRST so the bytes exist in storage before the CDN's eventual pull arrives. This preserves the Optimization #4 guarantee that legacy uploads get variants on first request — the CDN's origin pull (whatever its origin is) will find a 200 + WebP bytes, not a 404 race-condition.
- **What's NOT covered (deliberately)**: contracts (`/uploads/contracts/...`, `app.py:24490`) are intentionally NOT redirected and NOT given long Cache-Control. They're operator-uploaded business documents (signed PDFs for service bookings) that may want stricter handling later — private bucket, signed URL access, audit logging. A test in `test_cdn_uploads.py::TestContractsRouteUnchanged` pins this so a future blanket "redirect everything in /uploads/" edit can't silently regress it. CSS `background-image` sites — gallery cards, blog covers, custom sections — also aren't covered by frontend rewrite (same Optimization #4 limitation: srcset doesn't apply to background images). For those the redirect is the only protection until `image-set()` lands.
- **Frontend timing — first-paint correctness**: `IMG_UPLOADS_BASE` is a `let` module-global in `script.js` (line 5603) that defaults to `''` and gets populated from the page-bundle response BEFORE any render call (line 130, immediately after `await bundleRes.json()` and before the first `siteSettings = bundle.site_settings` unpack). This ordering is pinned by `tests/test_cdn_uploads.py::TestFrontendBackendCDNParity::test_js_assigns_before_any_render`. Without it, the first paint would render `/uploads/...` URLs that incur the redirect tax once, then subsequent re-renders would use the CDN URLs — visible jank if a CDN is configured.
- **Frontend / backend rule parity (continued from Opt #4)**: the JS `IMG_UPLOADS_BASE` assignment uses `replace(/\/+$/, '')` to strip trailing slashes, mirroring the Python `_uploads_public_base()` `.rstrip("/")`. The page-bundle field name (`uploads_public_base_url`) is identical on both sides. A drift-guard test class pins all of this so a one-sided edit can't silently break the optimization without breaking any functional behaviour.
- **Verification**: `tests/test_cdn_uploads.py` adds 29 cases across eight classes — `TestUploadsCDNRedirect` (6: redirect-when-set, filename round-trip preserved, trailing-slash normalization, no-redirect-when-unset, origin-pull header bypass, strict header value check), `TestVoiceCDNRedirect` (2: voice redirect + voice origin-pull bypass), `TestContractsRouteUnchanged` (1: pin that contracts stay on origin even when env set), `TestUploadsCacheControl` (2: existing-original gets 1-day public no-immutable, variant gets 30-day immutable — both `pytest.skip` if no fixture file is present), `TestPageBundleExposesCDN` (2: field present empty when unset, field present and trailing-slash-normalized when set), `TestSrcsetForCDN` (6: no-base relative, explicit base, env base auto-pickup, explicit `""` overrides env, trailing-slash strip, ineligible URLs return `''` regardless of base), `TestFrontendBackendCDNParity` (4: JS declares the global, JS reads the same field name, JS strips trailing slashes the same way, JS assigns BEFORE first render), `TestUploadsCDNPullSecret` (3: secret-unset falls back to value `1`, secret-set requires exact match, secret-set with no header still redirects), and `TestSelfLoopGuard` (3: same-host bypasses redirect, case-insensitive host match, different-host still redirects). Full suite now 178/178 (149 prior + 29 new), all green.

## Database Connection Pool Tuning (Optimization #9 — April 2026)

Ninth item from the perf list. Tier 7 added a process-wide `ThreadedConnectionPool` so we'd stop paying TCP+TLS+auth setup (~30-150 ms on managed Postgres) on every request, but its sizing was set at conservative leftover defaults: `min=1, max=10`. The boot log spelled it out plainly — `[db pool] initialised (min=1, max=10)` — and on a Replit autoscale deployment under bursty traffic those numbers were leaving real performance on the table. Optimization #9 retunes the defaults to `min=2, max=20`, keeps the env-var overrides for operators who need them, and pins the change with tests so a future "let's be conservative again" refactor can't silently undo it.

- **Why min=2 (was 1)**: Replit autoscale spins instances up on demand and traffic arrives in waves. With min=1, every cold worker has exactly one warm connection — the FIRST request of the wave gets it (instant), but the second concurrent request (very common: a page-bundle fanout fires multiple queries in parallel) has to open a fresh connection while the user waits. A 30-150 ms handshake tax on the second-fastest query is a typical real-world delay because that query is usually on the critical path for first paint. min=2 covers the common 2-parallel-queries case for free; bigger pages still get a brief handshake on the 3rd+ concurrent query but never on the first two.
- **Why max=20 (was 10)**: a single `/api/page-bundle` response can run 8-12 queries (site_settings, theme, contact, gallery, sphere, services, blog posts, voice settings, chatbot settings, custom sections, page metadata, …). With 5 concurrent users on a 10-cap pool, you're already at PoolError and falling through to the direct-connect fallback path — which works (the fallback is the whole reason `get_db()` doesn't ever raise) but is slower than just having had a bigger pool to begin with. 20 sits comfortably under managed-Postgres caps for a single app instance (most managed tiers allow 50-100 connections per database, and this is per-app-instance, not global).
- **Validation that's intentionally strict**: the `_resolve_pool_sizes(env=None)` helper (`app.py:488`) clamps min to >= 1 (psycopg2 requires it; max(1, ...) catches `DB_POOL_MIN=0` and negatives), then clamps max up to >= min (catches `DB_POOL_MAX=1` with the new default min=2; also catches the inverted-sizes misconfig `DB_POOL_MIN=10, DB_POOL_MAX=5`). Empty strings (the common operator mistake of `export DB_POOL_MAX=` with no value, which yields `""` not the default) fall back to the documented defaults via `or "<default>"`. Garbage non-int values like `DB_POOL_MAX=banana` deliberately raise at startup — silent fallback would hide the typo for weeks.
- **Refactor for testability**: the old code computed the constants inline at module scope with two `os.environ.get(...)` calls, which is impossible to unit-test without re-importing the module per test (slow, brittle, leaks state). `_resolve_pool_sizes(env=None)` is a pure function over a dict-like env that defaults to `os.environ` — the module-scope constants now come from a single call, and tests pass synthetic envs to cover every branch (defaults, overrides, clamping, empty strings, garbage). Side benefit: the docstring now lives next to the logic instead of in a comment block 200 lines away.
- **Operator override guidance — get the connection math right**: `DB_POOL_MIN` and `DB_POOL_MAX` env vars still take precedence over the defaults. The pool is **per process**, not per app instance — so under gunicorn with `--workers N`, the worst-case in-flight connections from a single instance is `N × DB_POOL_MAX`, not just `DB_POOL_MAX`. Multiply by the number of autoscale instances and ALSO leave headroom for the direct-connect fallback path (which can spill over when the pool is exhausted: each spillover request opens a fresh un-pooled connection that closes when the request returns). The full worst-case formula is `(num_instances × num_workers_per_instance × DB_POOL_MAX) + spillover_headroom`, and that whole sum needs to stay under your Postgres `max_connections` setting. On a small managed Postgres with `max_connections=50` and gunicorn `--workers 4`, the new default `DB_POOL_MAX=20` already implies 80 connections from a single instance — set `DB_POOL_MAX=10` (or fewer workers) to fit. Operators fronting Postgres with **PgBouncer** in transaction pooling mode get effectively-unlimited app-side pool capacity and should size `DB_POOL_MAX` based on per-request concurrency, not server connection caps. Operators on a tiny dev DB who want to minimize idle connections can set `DB_POOL_MIN=1` to opt back into the old behaviour explicitly.
- **Fail-loud on bad env values, with a friendly message**: garbage env values (`DB_POOL_MAX=banana`) raise a `ValueError` at module import via the `_parse_pool_int` helper, and the message names exactly which env var and which value is wrong (e.g. `[db pool] env var DB_POOL_MAX='banana' is not a valid integer. Set it to a positive integer (e.g. DB_POOL_MAX=20) or unset it to use the default (20).`). The alternative — silently falling back to the default — would hide a typo in deploy config for weeks; the friendly message turns startup-crash into actionable feedback. Pinned by `TestPoolValidation::test_garbage_error_message_names_var_and_value`.
- **What's NOT changed**: the `get_db()` fallback to `_direct_connect()` when the pool is exhausted or fails to initialise (Tier 7 design) is preserved verbatim — Optimization #9 just makes the fallback path get hit less often. The `_PooledConnection` subclass and its re-entrancy-safe `close()` are also untouched.
- **Verification**: `tests/test_db_pool_sizing.py` adds 16 cases across four classes — `TestPoolDefaults` (3: pure-function returns 2/20 on empty env, module-level constants match defaults when test env is clean, boot log format string still says `[db pool] initialised`), `TestPoolEnvOverrides` (4: DB_POOL_MIN honored, DB_POOL_MAX honored, both honored together, operator can opt back to min=1 for the old behaviour), `TestPoolValidation` (7: zero clamps to 1, negative clamps to 1, max-below-default-min clamps up, max-below-explicit-min clamps up, empty-string falls back to default, garbage int raises ValueError, error message names the offending var AND value for both DB_POOL_MIN and DB_POOL_MAX paths), and `TestPoolWiring` (2: `_init_db_pool` still references `_DB_POOL_MIN`/`_DB_POOL_MAX` so a refactor can't silently bypass the resolver, `_resolve_pool_sizes` is pure — same env in twice yields same result and caller env dict is not mutated). Live boot log on a fresh restart now reads `[db pool] initialised (min=2, max=20)`, confirming the change is observable end-to-end. Full suite now 194/194 (178 prior + 16 new), all green.

## Preconnect / DNS-Prefetch Hints (Optimization #10 — April 2026)

Tenth item from the perf list. The public homepage loads scripts and fonts from **six distinct third-party origins** synchronously in `<head>` (every cold visitor pays the full DNS+TCP+TLS handshake — typically 30–300 ms per origin — BEFORE the resource body even starts streaming): `unpkg.com` (Lucide icons), `cdnjs.cloudflare.com` (Three.js + DOMPurify), `cdn.jsdelivr.net` (Three.js CSS3DRenderer), `js.stripe.com` (Stripe SDK), plus `fonts.googleapis.com` and `fonts.gstatic.com` (loaded dynamically by `loadGoogleFont` in script.js once the theme settings come back). Without hints those handshakes happen serially as the HTML parser hits each `<script>` tag. Optimization #10 emits a block of `<link rel="preconnect">` + `<link rel="dns-prefetch">` hints as the FIRST thing inside `<head>`, so the browser fires all six handshakes in parallel with HTML parsing — by the time the parser reaches each script tag, the connection is already warm.

- **Where the hints live**: `public/index.html` carries a `<!-- PRECONNECT_HINTS_INJECT -->` placeholder right after the viewport `<meta>` tag (so it's the very first parser-discoverable element after charset/viewport). `serve_index()` in `app.py` calls `_build_preconnect_hints_html()` and replaces the placeholder server-side. Same pattern as `<!-- SEO_META_INJECT -->` and `<!-- THEME_VARS_INJECT -->`.
- **Why preconnect AND dns-prefetch (it's not redundant)**: preconnect (DNS+TCP+TLS) is the modern best practice — every current browser supports it. dns-prefetch (DNS only, the older hint) is left in as a no-cost fallback for the few legacy browsers that don't support preconnect; modern browsers ignore the dns-prefetch line when a matching preconnect is present, so listing both costs zero bytes after gzip and zero connections at runtime.
- **The fonts.gstatic.com `crossorigin` attribute is load-bearing**: the CSS spec mandates that `@font-face url(...)` font fetches go out as anonymous-CORS regardless of where they came from. A preconnect WITHOUT `crossorigin` warms a connection in the wrong (credentialed) connection pool — and when the actual font fetch fires, the browser opens a SECOND, fresh connection for the anonymous-CORS pool. The preconnect was wasted entirely: zero handshake savings, plus an extra unused socket sitting around. This is the #1 footgun in preconnect setups in the wild. The fonts.gstatic.com line includes `crossorigin`; the other five (which mirror plain non-CORS `<script>` tags) deliberately do NOT, because adding it would similarly mismatch the default credentialed-mode pool those scripts use. Pinned by `TestPreconnectHelper::test_fonts_gstatic_preconnect_has_crossorigin` AND `test_other_preconnects_do_not_have_crossorigin`.
- **Why right-after-`<head>` and not right-before-`</head>` for the legacy fallback**: the codebase is a CMS where admins can publish custom HTML designs from the Website tab; those snapshots are stored in `site_designs.html` and predate the placeholder. For backward compat `serve_index()` falls back to inserting the hint block when the placeholder is missing, BUT preconnect hints are position-sensitive — the browser only fires the handshake when the parser reaches the link tag. Inserting before `</head>` (the same fallback `theme_vars` uses) would put the hints AFTER the `<script src="https://unpkg.com/...">` tag, which means unpkg's own handshake started serially first and the optimization saved nothing. The fallback uses a regex `re.search(r"<head\b[^>]*>", ...)` so it handles `<head>` and `<head lang="en">` and so on, then injects right after the opening tag — guaranteed before any `<script>`. Theme vars don't need this because CSS variables resolve at compute time regardless of declaration order; preconnect hints fire on parse.
- **Maintenance contract**: `_build_preconnect_hints_html()` is a hardcoded string builder, NOT auto-introspected from script tags. The reasoning: the cost of a leftover preconnect for a removed origin is one unused TCP socket held open for ~10 seconds (negligible); the cost of a missing preconnect for a newly-added origin is a fully serial handshake on the critical path (significant). Err toward keeping the list comprehensive and updating it during code review when a new third-party `<script>` lands in `public/index.html`.
- **What this does NOT do**: it does NOT remove the actual `<script>` tags, defer them, or add `async`/`defer`/`fetchpriority` — those are separate optimizations. It also does NOT do `<link rel="preload">` (which actually fetches the resource into cache, costing bandwidth even if the resource isn't used). Preconnect is the lightweight "warm the socket" hint; preload is the heavyweight "start downloading now" hint. Six preconnects is comfortably within Chrome's recommended ceiling (~6–10 simultaneous preconnects before deprioritization kicks in).
- **List-drift detection (the silent-regression killer)**: the value of preconnect hints depends on the list staying in sync with the actual `<script>`/`<link>` tags in `public/index.html`. If a future PR adds a new third-party CDN (`<script src="https://newcdn.com/lib.js">`) without updating `_build_preconnect_hints_html()`, that origin silently pays serial handshakes on every cold visit and no test fails — the kind of drift that gets discovered six months later in a Lighthouse audit. `TestListDriftDetection::test_every_external_resource_origin_in_index_has_a_preconnect` walks `public/index.html` with a regex that extracts every `https://host` from `<script src=>` and `<link href=>` (NOT from anchor `<a href=>`, which are user-clickable links not resource fetches), and asserts every one has a matching preconnect in the helper. The companion test `test_extractor_actually_finds_the_known_origins` asserts the regex itself works (pins unpkg/cdnjs/jsdelivr/js.stripe), so a broken extractor can't make the drift test vacuously pass.
- **Verification**: `tests/test_preconnect_hints.py` adds 36 cases across four classes — `TestPreconnectHelper` (15: helper returns non-empty; preconnect for every origin × 6 parametrized; dns-prefetch fallback for every origin × 6 parametrized; fonts.gstatic.com has crossorigin; the other five do NOT have crossorigin), `TestServedHomepage` (16: GET / returns 200 with `<head`; served HTML contains preconnect for every origin × 6 parametrized; served HTML contains dns-prefetch fallback for every origin × 6 parametrized; fonts.gstatic.com crossorigin survives the round-trip; preconnect appears BEFORE the `<script src="https://unpkg.com/...">` byte position; placeholder comment is consumed not left as literal HTML), `TestLegacyDesignFallback` (3: helper is idempotent; legacy admin-saved design without placeholder still gets all six preconnects AND they appear before the parser-blocking script; regex fallback handles `<head>` with attributes like `lang="en"`), and `TestListDriftDetection` (2: every external `<script>`/`<link>` origin in `public/index.html` has a matching preconnect; the extractor itself works). Live curl of `GET /` confirms the 12-link block lands on line ~53 of the served HTML, well before any `<script>`. Full suite now 230/230 (194 prior + 36 new), all green.

## JS Bundle + Minification (Optimization #11 — April 2026)

Eleventh item from the perf list. The public homepage loaded **two** unminified JS files on every cold visit — `public/script.js` (351 KB, the main framework) and `public/voice.js` (53 KB, the proactive voice agent) — for **404 KB across two separate requests**. Even with Brotli already compressing both files (the existing tier-8 compression middleware), two requests means two round-trips of HTTP overhead and two header sets, and unminified JS still ships ~40% of byte weight as comments + indentation that compress poorly compared to minified output. Optimization #11 concatenates both files in their original load order, runs the result through `rjsmin` (pure-Python conservative whitespace+comment stripping), and serves the output as a single content-fingerprinted blob from `/bundle.{sha256[:12]}.min.js` with a one-year `Cache-Control: public, max-age=31536000, immutable` header. Net result on the current sources: **42.9% smaller pre-Brotli (404 KB → 231 KB), 34% smaller post-Brotli (88 KB → 59 KB on the wire), and one fewer round-trip per cold visit**.

- **Where the bundle lives**: `asset_bundle.py` (new module). `BUNDLE_SOURCES = ["script.js", "voice.js"]` is the ordered source list — order matters because `script.js` defines globals (e.g. `BUILTIN_SECTION_MAP`, `loadGoogleFont`) that `voice.js` reads at init. `build_bundle()` reads each source, joins them with a `\n;\n` ASI-safe separator (prevents the last line of one file from accidentally fusing with the first token of the next if the next starts with `[ ( + - / \``), runs `rjsmin.jsmin()`, computes a 12-char SHA-256 prefix of the minified bytes, and stashes `{hash, content_bytes, url, raw_size, min_size, sources}` in a module-level dict guarded by a `threading.Lock`. `get_bundle()` is the lazy accessor — first call builds, subsequent calls are O(1). The whole thing is in-memory only: no filesystem writes, no derived artifacts to gitignore, no race conditions across multi-worker deployments (every worker independently builds the same deterministic bundle on boot, paying ~50 ms of cold-start CPU).
- **Why content-fingerprint and not query-string cache-bust**: a fingerprinted URL like `/bundle.87a915a9bc9a.min.js` lets the response carry `Cache-Control: max-age=31536000, immutable` honestly — the URL is permanently bound to the exact bytes it serves, so revalidation is provably unnecessary. A `?v=` query string is also widely supported but has two problems: (1) some intermediate caches and CDNs ignore query strings entirely for cache-key purposes (cf. Cloudflare's "Standard" cache level), and (2) it forces every client to revalidate on URL change instead of allowing the browser to drop the old asset from cache the moment its URL stops being referenced anywhere. With a fingerprinted path, the natural HTTP semantics handle invalidation: when the source files change → hash changes → URL changes → cached HTML pointing at the old URL silently 404s on first reload, the page refetches HTML, the new HTML carries the new URL, browsers fetch the new bundle exactly once. No middleware logic, no cache-busting libraries, no version-bump rituals.
- **Why 404 instead of "always serve current bundle for any hash"**: the route is `/bundle.<bundle_hash>.min.js` and explicitly compares the URL hash against the in-memory bundle hash. A wrong-hash request returns 404, NOT the current bundle. The reasoning: if a stale-hash request silently served the current bundle, the stale URL would itself get cached for a year (`immutable` header), permanently pinning a dead URL as an active asset on edges and clients. 404 forces the client to refetch HTML, which carries the new hash, which is the only URL that gets the long-cache header. A defensive sub-property — pinned by `TestBundleServing::test_wrong_hash_404_body_does_not_leak_current_hash` — is that the 404 body doesn't echo the current valid hash, so an attacker can't probe the exact hash via a single guess request.
- **Where the script tag gets injected**: `public/index.html` carries a `<!-- JS_BUNDLE_INJECT -->` placeholder at the bottom of `<body>` (replacing the original two `<script src="/script.js">` and `<script src="/voice.js">` tags); `serve_index()` in `app.py` calls `asset_bundle.bundle_script_tag()` and replaces the placeholder server-side. Same disk/db dual-path as `<!-- THEME_VARS_INJECT -->`, `<!-- LOADING_NAME_INJECT -->`, and the others — admin-saved designs in `site_designs` predate the placeholder and inline the original two script tags directly, so the fallback is a regex rewrite that swaps the first `<script src="/script.js"></script>` for the bundle tag and removes any subsequent `<script src="/voice.js"></script>`. Both tags are removed, never duplicated. The injection block is wrapped in a `try/except` that logs and falls back to no-bundle injection on failure — at which point legacy designs still load via their original tags, and the disk template loudly fails the `TestHomepageInjection` tests rather than silently degrading in prod.
- **Source files stay served**: the original `/script.js` and `/voice.js` URLs continue to serve the unminified source from the existing `serve_static()` route — the bundle is an OPTIMISED VIEW of the same code, not a replacement. Two reasons: (1) admin-saved designs in `site_designs.html` may inline `<script src="/script.js">` and `<script src="/voice.js">` directly, and those have to keep working until the next admin re-publish picks up the placeholder; (2) developers debugging in browser devtools find the unminified source easier to read. `TestSourceFilesStillServe::test_script_js_unchanged_size` pins that `/script.js` keeps serving 359,724 bytes (the on-disk size) — if a future change accidentally starts serving the minified blob from `/script.js`, this fires loudly.
- **Why rjsmin and not terser/esbuild/uglify**: terser and esbuild are the industry-standard JS minifiers and would yield deeper savings (typically 50-65% pre-gzip via name mangling and dead-code elimination), but they require a Node.js toolchain. This project's `package.json` is a 308-byte stub used only for a tiny dev-tooling surface; adding a Node build step to the Dockerfile + alembic migration container would meaningfully grow the image size and add a second runtime to keep patched. `rjsmin` is pure Python (zero new system deps), conservative (whitespace + comment stripping, no name mangling, no AST rewrites), deterministic across runs, well-tested, and handles every modern syntax feature `script.js` uses (220 template-literal interpolations, 7 optional-chain expressions, 17 async functions, 34 awaits, 2 spread operators) without corrupting them. The 42% size drop with rjsmin is comfortably inside the perf-list target range (30-60%) and the post-Brotli-on-the-wire saving is the same order of magnitude as a more aggressive minifier would achieve, because Brotli already eliminates most of what name mangling would compress.
- **List-drift detection**: the value of bundling depends on `BUNDLE_SOURCES` staying in sync with the actual `<script src="/...">` tags in `public/index.html`. If a future PR adds a third local JS file (e.g. `<script src="/chat.js">`) without adding it to `BUNDLE_SOURCES`, that file would load as its own request, the bundle would silently regress to a partial-coverage optimization, and no test would fail. `TestBundleListDriftDetection::test_every_local_script_src_in_index_is_bundled` walks `public/index.html`, regex-matches every `<script src="/file.js">` (local paths only — `https://` CDN scripts belong to Optimization #10), and asserts every one is in `BUNDLE_SOURCES` (with an explicit exception for the bundle URL itself, since it's the OUTPUT not a source). The companion test `test_every_bundled_source_exists_on_disk` catches the inverse drift — `BUNDLE_SOURCES` listing a file that no longer exists, which would crash `build_bundle()` on first call.
- **What this does NOT do**: it does NOT add `defer`, `async`, `module`, or `fetchpriority` attributes (those are separate page-load optimizations); it does NOT split into multiple bundles or do code-splitting (single-bundle is fine for 231 KB minified — code-splitting wins start above ~500 KB); it does NOT touch CDN-hosted libraries (`lucide`, `three.js`, `dompurify`, `stripe` — those are unchanged and benefit from Optimization #10's preconnect hints); and it does NOT introduce source maps (would re-add 200+ KB of byte weight that admins would never download anyway, and the unminified source files remain available at their original URLs for devtools debugging).
- **Verification**: `tests/test_js_bundle.py` adds 43 cases across seven classes — `TestBundleBuild` (12: state populated; hash format is 12 hex chars; URL format; minified < raw; ≥20% size reduction floor; sources list matches constant; hash stable across rebuilds; 4 critical identifiers preserved × parametrized [`loadGoogleFont`, `imgSrcset`, `BUILTIN_SECTION_MAP`, `DOMPurify`]; template-literal interpolations preserved; async/await keywords survive; `get_bundle()` is lazy + idempotent), `TestBundleServing` (7: 200 status; `application/javascript` Content-Type; `max-age=31536000, immutable` cache header; `Vary: Accept-Encoding`; body bytes match in-memory; wrong-hash 404; 404 body doesn't leak current hash), `TestSourceFilesStillServe` (3: `/script.js` and `/voice.js` both still serve; `/script.js` unchanged size pin), `TestHomepageInjection` (4: served HTML contains bundle URL; placeholder consumed not literal; no leftover `<script src="/script.js">` or `<script src="/voice.js">`; bundle tag is well-formed `<script src="..."></script>`), `TestLegacyDesignFallback` (11: legacy admin-saved design with both tags gets bundle injected via regex; design with only `script.js` tag still gets the bundle; AND 9 parametrized variants pinning that the hardened regex tolerates `defer`/`async`/`type=`/`crossorigin` attributes in any order, single quotes, whitespace around `=`, cache-bust query strings, and mixed-case `<SCRIPT>` tags — without these, real-world admin designs that aren't byte-identical to `<script src="/script.js"></script>` would silently miss the optimisation), `TestBundleListDriftDetection` (2: every local `<script src=>` in `public/index.html` is in `BUNDLE_SOURCES`; every `BUNDLE_SOURCES` entry exists on disk), and `TestProductionRegression` (2: URL pattern shape pinned to `/bundle.[12-hex].min.js`; ≥25% byte savings + request-count drop pinned with informative failure message that distinguishes "rjsmin bypass" from "source-code densification"). The 20%/25% floors are deliberately lower than the actual ~42% delivered today — they are catastrophic-regression detectors, not optimisation policy. Live curl confirms `/bundle.{hash}.min.js` returns 200 + immutable cache + 236,449-byte body; with `Accept-Encoding: br` the body drops to 58,747 bytes on the wire. Browser console shows `voice.js`'s init message firing correctly (proving both source files' code runs from the bundle) with zero JS errors. Full suite now 322/322 (279 prior + 43 new), all green.

## VELO Master Communication Surface (April 28 hotfix)

A real master operator reported "no tools discovered" + a 405 wall when invoking admin_ai. The audit log made the failure modes concrete: rows showed VELO Master sending `command=""` and `command="list"` (the standard tool-gateway capability-discovery probes) and getting back our 400 "unknown command" — even though we have 31 registered handlers. A separate trace showed `POST /api/velo/admin_ai` returning a literal 405 because the master was auto-deriving per-agent URLs that don't exist on this app, and Flask's homepage catch-all (which is GET-only) was returning 405 to the master with no hint about what URL it should be using instead. Three concrete failure modes, all silent until the master couldn't make any progress.

The fix lives entirely in `velo_endpoints.py` (no schema, no client code, no master-side changes — purely server-side route surface).

- **`_DISCOVERY_ALIASES` constant** (top of file, just below `_command_handlers`): a frozenset of 13 strings every modern tool gateway might send to probe for capabilities — `""`, `"list"`, `"list_commands"`, `"list_tools"`, `"tools"`, `"discover"`, `"discovery"`, `"capabilities"`, `"help"`, `"describe"`, `"?"`, `"ls"`, `"show"`. Matched after `.strip().lower()` so case + whitespace variants work too. Centralised so the alias list is the single source of truth for the discovery fast-path AND its sync-test (`test_alias_set_matches_module_constant`) catches drift between the test list and the real constant.

- **`_discovery_payload()` helper**: the single canonical capability-discovery payload returned by FOUR surfaces — POST `/command` with a discovery alias, GET `/command`, GET `/tools`, GET `/capabilities`. Builds fully-qualified URLs from `request.host_url` so the master never has to guess base paths or auto-derive per-agent URLs. Includes `agent_ids: ["admin_ai", "visitor_ai"]`, the full sorted commands list (with description / params_schema / requires_confirmation per command), and a `usage` block with three human-readable strings (structured_command, free_text_chat, discovery) literally describing how to call each surface. The "drift between discovery surfaces is a silent failure" risk is pinned by `test_all_discovery_surfaces_return_same_command_list`.

- **`/command` accepts GET + POST**: GET returns the discovery payload directly. POST keeps existing behaviour but adds the discovery-alias fast-path — `command.strip().lower() in _DISCOVERY_ALIASES` returns 200 + payload. **Discovery hits are deliberately NOT written to `velo_audit_log`** (high-frequency master polling, fully deterministic response, zero per-call context worth keeping) — the original implementation logged each one under a synthetic `__discover` name to keep them filterable, but the architect flagged that even filterable rows still bloat the table at scale, so the audit call was removed entirely. If per-poll audit ever becomes useful (rate-limit triage, master-fingerprint tracking) it can be re-added under a synthetic name; the comment above the fast-path documents the trade-off.

- **`/chat` accepts GET + POST**: GET returns a usage hint with the `body_schema`, `agent_ids`, and `discovery_endpoint` URL — so a master probing `/chat` for connectivity gets self-describing JSON instead of the original 405. POST behaviour is unchanged.

- **Improved unknown-command 400 response**: now includes `hint` (literal string telling the master to POST `{message, agent_id}` to `/api/velo/chat` for free-text and to GET `/api/velo/tools` to list commands), `chat_endpoint` URL, `command_endpoint` URL, and the existing `available_commands` list. The hint specifically calls out the case where master sent the AGENT NAME as the command (a common mistake — pinned by `test_unknown_command_when_master_treats_agent_as_command`).

- **GET `/tools` and GET `/capabilities` aliases**: same handler (`list_tools`), both returning the canonical discovery payload. Auth-gated like `/status` because confirmation-gated command descriptions hint at the destructive surface.

- **Wildcard `velo_unmatched`**: a `/<path:rest>` catch-all on the velo blueprint accepting GET / POST / PUT / PATCH / DELETE that returns 404 JSON with the full endpoint map (each endpoint's method + URL) and the `agent_ids` list. NOT auth-gated — this is a self-describing routing hint identical in spirit to a 404 page, and a master that hasn't authed yet still needs to know what URLs exist so it can authenticate against them. Pinned by `TestVeloUnmatchedRoutes` (7 cases including the per-agent-per-tool URL pattern `/api/velo/admin_ai/web_search` master might auto-derive). Routing precedence: Flask prefers the more specific route, so all the named routes (`/command`, `/chat`, `/status`, `/tools`, `/capabilities`, `/refresh-registration`) still win against the wildcard for the methods they're registered with. The wildcard only fires for paths none of the named routes recognise OR for known paths called with an unsupported method (`PUT /command`, `POST /status`, `DELETE /tools`, etc) — that second case is a deliberate semantic choice: instead of Flask's stock 405 (which carries no hint about which method IS allowed), the master gets a self-describing 404 with the full endpoint-map listing each path's method, so it can self-correct on the next call instead of going into another guessing round. Pinned both ways: `test_known_routes_still_win_over_wildcard` + `test_status_route_unaffected_by_wildcard` lock the supported-method case; `test_unsupported_method_on_known_path_resolves_to_wildcard_404` (parametrized over 7 path×method combinations) locks the wrong-method case so a future "let's restore HTTP-spec 405" refactor surfaces as a deliberate test edit rather than a silent semantic flip.

- **`AGENT_IDS` module constant** (top of `velo_endpoints.py`, just below `_DISCOVERY_ALIASES`): single source of truth for the `["admin_ai", "visitor_ai"]` list used by `_discovery_payload()`, by GET `/chat`'s usage hint, and by the wildcard 404 hint. Previously the same list was duplicated inline in three response bodies plus `register_with_velo()` in `app.py` — exactly the kind of three-place-edit drift magnet that goes silently stale when a third logical agent gets added. `test_agent_ids_constant_used_consistently` asserts all three response surfaces return the same list as the imported constant, so an inline-literal regression in any one surface is caught immediately. (The boot-time registration in `app.py` is left as-is for now — a separate pre-existing literal not on the hot request path; future cleanup if a third agent ever lands.)

- **Tests in `tests/test_velo_master_communication.py`** — 49 cases across five classes: `TestDiscoveryProbes` (22: every alias × parametrized + canonical URL shape + agent_ids + usage hints + alias-set drift guard), `TestUnknownCommandHint` (2: hint contains `/chat` URL + agent-as-command mistake), `TestDiscoveryByGet` (5: GET surfaces × 3 + GET `/chat` returns usage not 405 + drift between surfaces), `TestVeloUnmatchedRoutes` (18: 7 unknown paths × method × parametrized + wildcard routing precedence + `/status` unaffected + wildcard not auth-gated + 7 parametrized wrong-method-on-known-path cases pinning the deliberate 404-with-hint behaviour + 1 AGENT_IDS-constant drift guard across all 3 surfaces), `TestProductionRegression` (3: literal replays of the audit-log entries that originally surfaced the bug — `cmd=''`, `cmd='list'`, `POST /api/velo/admin_ai`). Live curl verified all original failure modes are fixed: GET `/api/velo/admin_ai` was 200 HTML → now 404 JSON; POST `/api/velo/admin_ai` was 405 → now 404 JSON; POST `/command` empty was 400 → now 200 with 31 tools; GET `/tools` was 200 HTML → now 200 JSON with 31 tools.

- **`tests/conftest.py` adds `velo_auth_headers` session fixture**: builds `{Authorization: Bearer <VELO_AGENT_KEY>, Content-Type: application/json}` from the test env. Necessary because `VELO_AGENT_KEY` is exposed as a deployment secret and leaks into the pytest env, so `verify_velo_key()` rejects unauthenticated calls with 401 — every velo test now sends this fixture in `headers=` so it actually reaches the handler logic. Single source of truth so a future key-rotation only needs one edit.

- **What this does NOT change**: `register_with_velo()` in `app.py` is unchanged (the boot-time three-field callback URL trick was already correct — master apparently ignored those fields, hence the per-agent-URL fallback and the catch-all fix). `velo_handlers.py` and the 31 existing `@velo_command` registrations are untouched. `velo_client.py` (outbound calls FROM this app TO the master) is untouched. Full suite now 279 (230 prior + 49 new), all green.

## Admin Performance Tab + Variant Auto-Cleanup (April 28 — UX follow-up)

Two operator-facing improvements layered on top of the existing perf optimisations: (1) WebP variants now self-evict when an admin re-uploads a same-named source image, and (2) a new "Performance" tab in the admin dashboard surfaces the runtime state of the six site-perf optimisations on one screen + adds a single "Regenerate ALL variants" button for the cases the auto-eviction can't catch. The motivation: the operator-developer asked for self-service site management — fewer "ssh in and check" round-trips, less waiting for an engineer to debug "why is the old hero image still showing".

- **`image_optimize.delete_variants(filename)`** (added at line ~264 in `image_optimize.py`, right after `generate_webp_variants`): given a source filename like `abc.jpg`, deletes `abc-400.webp`, `abc-800.webp`, `abc-1600.webp` from the storage backend if present. Pure file I/O — no DB writes. Returns the count actually deleted (so callers can log it). Short-circuits on non-{jpg,jpeg,png} extensions (vector files, GIFs, already-WebP files have no derived variants by construction). Each per-width delete is wrapped in its own try/except — if the backend errors on one width (transient S3 503, local I/O hiccup), the function logs and continues to the next width rather than bubbling the exception. Partial cleanup is strictly better than blocking the upload over a transient backend error; the next upload of the same name (or the bulk regenerate button) will clean up the orphan. Idempotent: a second call on a stem with no variants returns 0 and never raises, so the upload handler can call it unconditionally on every upload without conditional logic.

- **Wired into both upload handlers** in `app.py`: `admin_upload_image` (~line 20212) and `admin_upload_media` image branch (~line 20292) both now call `image_optimize.delete_variants(unique_name)` immediately BEFORE `image_optimize.generate_webp_variants(unique_name)`, inside the existing try/except. The order matters: delete then regenerate ensures the new source's variants are written fresh, NOT a partial overwrite of the stale ones (which would leave width breakpoints from an old aspect ratio mixed with new ones — the actual production bug this fixes). Why this is essential: variants are served with `Cache-Control: max-age=31536000, immutable`, so a browser that fetched the OLD `abc-800.webp` keeps using it for a full year regardless of source updates — without auto-eviction, replacing `abc.jpg` would silently leave every visitor seeing the old hero image until their cache expired or the file got renamed. Now the new variants are written under the same path with new bytes, and any browser that re-validates gets the new version (or the cache key gets evicted when the source URL changes via re-upload).

- **`/admin/api/performance` (GET, admin-only)** + helper `_collect_performance_stats()` (added at line ~20336 in `app.py`): returns one JSON snapshot with six top-level sections — `bundle`, `images`, `db_pool`, `cdn_uploads`, `api_cache`, `preconnect`. Each section is gathered by its own helper (`_stats_bundle`, `_stats_images`, etc.) wrapped in try/except, so a broken backend in one section (e.g. DB unreachable for the image-count query) returns `{"error": "..."}` for that section but doesn't 500 the whole page. The six sections are deliberately the same six covered by Optimization #3-#11 — bundle stats from `asset_bundle.get_bundle()`, image counts from a `SELECT COUNT` on `uploaded_images` plus a local-only filesystem walk distinguishing originals from variants, pool config from the `_DB_POOL_MIN`/`_DB_POOL_MAX` module vars + env-var-set indicators, CDN base URL from `UPLOADS_PUBLIC_BASE_URL`, API cache endpoint count from the `_CACHEABLE_API_PATHS` set, and preconnect origins parsed back out of `_build_preconnect_hints_html()` (so the listing surfaces drift if the helper ever changes). The "parse the helper's output" approach for preconnect is intentional: it means the dashboard can never disagree with what the homepage actually serves.

- **`/admin/api/performance/regenerate-image-variants` (POST, admin-only, local-storage-only)**: walks every file at the uploads root, skips variant filenames (matched via `image_optimize.is_variant_filename`) and non-image extensions, then calls `delete_variants` + `generate_webp_variants` on each source. Returns counts: `regenerated` / `skipped` (decoded but produced no variants — typically sub-400px thumbnails) / `errors` / `total_scanned`. Synchronous in the request handler — typical small library (~50 images) finishes in 1-3 seconds; the UI disables the button + shows "Regenerating…" while it runs. **Local-storage backend ONLY** — for S3 backends the endpoint returns 400 with the message "Bulk regenerate is local-storage-only. S3 backends should script per-file regeneration." The reasoning: enumerating an S3 bucket of 100k images would time out the request and rack up egress; the operator-developer running on local backend gets a single button, S3 deployments get a documented escape hatch via scripting. Use cases the auto-cleanup can't catch: source files replaced by shell / snapshot / object-store sync (the write didn't go through the upload route); `_RESPONSIVE_WIDTHS` changed in `image_optimize.py` (variants at the OLD widths now orphaned, NEW widths missing); WebP encoder upgrade; Pillow upgrade.

- **Performance tab UI** (`templates/admin/dashboard.html`): a new sidebar button "Performance" in the Insights group (right after "Custom Dashboards"), a tab-content panel with a CSS grid of six cards (one per stat section), and a JS block (`loadPerformance`, `_perfRenderCards`, `_perfFmtBytes`, `_perfCard`, `_perfRow`, `regenerateImageVariants`). Cards are rendered via small string-template helpers — no React, no framework, just template literals + the existing `escapeHTML` global. Each card has a title labelling which optimisation it covers (e.g. "JS Bundle (Opt #11)", "Image Variants (Opt #4 / #5)") so the operator can correlate the dashboard with the docs in this file. The "Regenerate all variants" button only renders when `images.regenerate_supported === true` (i.e. local backend), with an inline note explaining why it's hidden on S3. CSRF is auto-injected by the existing fetch wrapper (no manual `X-CSRF-Token` needed).

- **Verification**: `tests/test_admin_performance.py` adds 18 cases across three classes — `TestDeleteVariants` (10: returns 0 when none exist; deletes existing variants and reports count 3; idempotent; only touches matching stem; skips non-image extensions × 4 parametrized; survives single-width delete failure with partial-count return), `TestPerformanceStatsRoute` (5: anon caller blocked with 302/401; returns expected six-section shape; bundle section has `raw_size_bytes`/`min_size_bytes`/savings consistency; db_pool section reports configured min/max; api_cache lists known endpoints including `/api/page-bundle`), `TestRegenerateVariantsRoute` (4: anon caller blocked; authed POST without CSRF token returns 403 csrf_failed; non-local backend (S3) returns 400 with `backend` marker and "local-only" hint; local backend returns counts and actually creates variants for a synthetic seeded source). The flaky-delete test uses `monkeypatch` to inject a one-shot exception into `storage.delete` for a specific filename — pins the partial-cleanup contract so a future "let's bubble the exception" refactor surfaces as a deliberate test edit rather than a silent semantic flip. The non-local-backend test monkeypatches `storage.get_storage().name = "s3"` so the contract is verified without standing up a real S3 backend. Full suite: 297/297 (279 prior + 18 new), all green. The existing `test_image_optimize.py` (55 cases) and `test_js_bundle.py` (43 cases) still pass — no regression on the upload/serve paths or bundle building.

- **What this does NOT do**: it does NOT change any database schema (no new tables, no ALTER, no migrations); it does NOT auto-poll (loadPerformance fires only on tab open + the explicit Refresh button — this is a diagnostic page, not a live dashboard); it does NOT support S3 bulk regenerate (deliberate scope cut — S3 deployments script it instead); it does NOT delete the uploaded source image during variant cleanup (only the `<stem>-{400,800,1600}.webp` derived files); and it does NOT add per-environment variant counts for non-image media (audio/video files have no WebP derivatives by construction).

## Admin Developer Console (April 28 — operator self-service)

A new "Developer" admin tab combining (a) an embedded **runbook** of "when you change X, do Y" accordion cards with one-click action buttons, and (b) a **service-health** panel that probes every external provider on demand. The motivation: same operator-developer who asked for the Performance tab also wanted "fewer 'is the API key still good?' Slack pings to me" + "stop emailing me when something silently breaks after a deploy". The runbook captures the dozen or so footguns that previously lived only in this file (or in nobody's head); the health panel turns "is Resend wired up?" into a button, not a curl invocation.

- **`devconsole.py`** (new, ~330 lines): the catalog + probes for nine external providers (`openai`, `anthropic`, `twilio`, `resend`, `elevenlabs`, `brave_search`, `yelp`, `google_places`, `tripadvisor`). The exported names are `PROVIDERS` (ordered list, the contract with the frontend for which Service Health cards render), `_is_configured(name)` (pure env-var check, no network), `provider_summary()` (returns `[{name, configured}, ...]` for tab-load — zero network calls so it's safe to refresh aggressively), and `probe(name)` (the dispatcher that fires the actual probe). Each `_probe_*` helper picks the cheapest free-tier endpoint that proves the credential works: `openai`/`anthropic` hit `GET /v1/models`, `twilio` hits `GET /Accounts/{SID}.json` with HTTP basic auth, `resend` hits `GET /domains` (so it also surfaces "domain not verified" indirectly), `elevenlabs` hits `GET /v1/user`, `brave_search` does a one-result query, `yelp` hits `GET /v3/businesses/search?limit=1`, `google_places` does a Find Place text query, `tripadvisor` does a tiny location search. Every probe goes through `_http_probe(method, url, headers, params, timeout=8.0)` which catches `httpx.TimeoutException` + `httpx.HTTPError` and returns the standard `{ok, latency_ms, error}` shape — a probe NEVER raises out to the route, even on DNS failure / SSL error / JSON decode error / unexpected status. The `probe()` dispatcher itself wraps the per-provider call in a final try/except so a developer-introduced bug in a probe (e.g. KeyError on a renamed env var) still surfaces as a structured `"probe crashed: ..."` error rather than a 500. Provider names are lowercase snake_case throughout — frontend/backend/test catalog all share the same string set.

- **Probe-error redaction (`_redact` + `_collect_secrets`)**: two of the nine providers (Google Places, TripAdvisor) pass the API key as a `?key=...` query param rather than via an HTTP header — that means the full request URL embedded inside an `httpx.HTTPError` `str()`, OR echoed back inside a 401 response body, would otherwise leak the key into the JSON returned to the admin browser. Defended in two layers: (1) before each request, `_collect_secrets` extracts every value the probe is about to send that's known to be sensitive — values whose param name is in `_SECRET_PARAM_KEYS = {key, api_key, apikey, token, access_token, auth}`, header values whose name is in `_SECRET_HEADER_KEYS = {authorization, x-api-key, xi-api-key, x-subscription-token}` (with the bare token portion of `Bearer xxx`/`Basic xxx` also collected so a body that echoes only the token half is still masked), and both halves of any HTTP-basic-auth tuple — and the result is passed to `_redact` after error formatting; (2) belt-and-suspenders, `_redact` also runs a regex pass that replaces the entire `?query` portion of any URL embedded in the error message with `?[REDACTED]`, so even an unrecognised secret format (a future provider that uses a non-standard param name) still doesn't leak via URL embedding. The 6-character minimum on literal masking prevents accidentally eating common short substrings of error messages. Three regression tests pin both layers (`TestRedaction`) plus three end-to-end tests (`TestProbeDoesNotLeakKey`) that mock `httpx.Client` to deliberately leak a canary key in both the exception path and the 4xx body path then assert the canary doesn't appear in the route's JSON response.

- **Five admin routes in `app.py`** (added at line ~20572, immediately after `_stats_preconnect` in the performance section): `GET /admin/api/devconsole/snapshot` returns `{providers: [...], runbook_state: {...}}` where `providers` is the no-network summary and `runbook_state` is a small bundle of read-only state the runbook cards substitute via `{state.X}` placeholders (image counts and `image_variants_supported` reused from `_collect_performance_stats()`, plus `preconnect_count`, `bundle_source_count`, `cacheable_endpoint_count`, `feature_flag_count` from `_FEATURE_REGISTRY`, plus `admin_email_set`/`admin_phone_set`/`velo_master_url_set`/`velo_agent_key_set`/`scheduler_loaded`/`site_url`). `POST /admin/api/devconsole/test-provider` body `{"provider": "..."}` validates the name against the catalog (returns `{error, known: [...]}` 400 on mismatch so a typo in the frontend surfaces with the valid set already in the response, not a silent dispatch to nothing) then returns `probe()`'s output with `provider` echoed back. `POST /admin/api/devconsole/test-email` and `POST /admin/api/devconsole/test-sms` are deliberately **fixed-recipient** — they ALWAYS send to `ADMIN_EMAIL` / `ADMIN_PHONE` from env, never accept a recipient field — so even if the admin session leaks the button can't be repurposed as an outbound spam relay. Both return 400 if the env var isn't set (clean prerequisite error, no provider call wasted), and 502 with the provider's error message on `MessagingError` (so a stale Resend key shows "Resend HTTP 401: invalid api key" inline, not a generic 500). The test-SMS route also writes a `sms_cost_events` ledger row (`surface="sms_devconsole_test"`) after a successful Twilio send so the cost dashboard accounts for the (real, billable) test sends instead of silently under-counting; `record_sms_cost` swallows its own exceptions, so a ledger write failure can't 500 the button. `POST /admin/api/devconsole/reregister-velo` calls `register_with_velo(force=True)` so a stale in-process "already registered" flag doesn't suppress the call (the whole point of the button is to re-register after rotating `VELO_AGENT_KEY` or pointing at a new master). All five routes are `@admin_required`; POSTs are covered by the existing CSRF middleware (no extra wiring).

- **Developer tab UI** (`templates/admin/dashboard.html`): sidebar button "Developer" in the Insights group right after "Performance" (~line 1835), tab-content panel with two sections — **Runbook** (10 collapsed-by-default accordion cards organized into four categories: Images & Media × 3, Site Performance × 3, Integrations & Secrets × 3, Deployment & Operations × 1) and **Service Health** (one card per provider with configured/not-configured badge + "Test now" button + inline "✓ OK · 234 ms" / "✗ Failed · {error}" result). Runbook card definitions live in a single `_DEV_RUNBOOK_CARDS` JS array — each entry has `id` / `category` / `title` / `body` (HTML, supports `{state.X}` placeholders that get substituted from `runbook_state`) / `actions` (zero or more buttons with `label` / `fn` (window-global function name) / `kind` (`primary`|`secondary`|`danger`) / optional `disabledIf` (state key that must be truthy) / `disabledMsg` (alert text when disabled)). Action buttons stop the click from bubbling so they don't toggle the accordion. The "Regenerate variants" actions link to the Performance tab (`_devGotoPerformance` clicks the existing tab button) and optionally fire its existing regen button after a 250ms render delay (`_devGotoPerformanceAndRegen`) — no duplicated business logic. The 10 runbook cards cover: stale variants after shell/snapshot/sync replace; changed `_RESPONSIVE_WIDTHS`; new image upload route (must call `delete_variants` before `generate_webp_variants`); new third-party script (must add to `_build_preconnect_hints_html`); new local JS file (must add to `BUNDLE_SOURCES`); new public read-only API (consider adding to `_CACHEABLE_API_PATHS`); rotated provider secret (workflow restart needed, then test); new email template / sender domain (Resend silently drops unverified DKIM); new SMS template / Twilio number (Twilio bills per 160-char segment, emoji forces unicode encoding); new domain or rotated `VELO_AGENT_KEY` (set `SITE_URL`, verify Resend domain, update Twilio webhooks, re-register agent). All test-IDs follow the `{action}-{target}` pattern (`tab-developer`, `button-developer-refresh`, `card-runbook-{id}`, `toggle-runbook-{id}`, `button-runbook-{id}-{fn}`, `card-health-{name}`, `button-test-{name}`, `text-health-result-{name}`).

- **Verification**: `tests/test_admin_devconsole.py` adds 46 cases across nine classes — `TestProviderCatalog` (2: pinned set of nine names, summary shape contract); `TestIsConfigured` (9: six single-env-var providers parametrized, plus twilio's both-keys-required rule, plus openai's canonical-or-Replit-alias acceptance, plus unknown-name returns false); `TestRedaction` (4: literal-secret masking, URL-query-string stripping, 6-char-floor protection, empty-secret-list still strips queries); `TestProbeDoesNotLeakKey` (3: Google Places key in URL doesn't appear in `httpx.HTTPError` path, TripAdvisor key in echoed 4xx body is masked, Resend bearer token in echoed 4xx body is masked — each uses a `_FakeClient` patched onto `httpx.Client` to deliberately leak a canary string and asserts the canary is absent from the route's JSON); `TestProbe` (3: unknown provider returns structured error not crash, unconfigured provider short-circuits without network, dispatcher wraps inner exceptions with `"probe crashed"` prefix); `TestSnapshotRoute` (4: anon blocked, authed returns expected shape with all 13 runbook_state keys, `admin_email_set` flag tracks env var change live, **plant-canary-in-every-secret-env-var test confirms the snapshot JSON contains zero secret values**); `TestTestProviderRoute` (5: anon blocked, CSRF required, missing `provider` field 400s, unknown provider 400 with `known` list in response, mocked probe path returns provider field); `TestTestEmailRoute` (6: anon blocked, CSRF required, `ADMIN_EMAIL` unset 400, success path uses messaging helper with recipient locked to env, `MessagingError` returns 502 with provider error in body, **JSON body with `to`/`recipient`/`email` fields is ignored — recipient is always `ADMIN_EMAIL`**); `TestTestSmsRoute` (7: same six cases pinned to `ADMIN_PHONE` plus one regression case asserting the success path writes a `sms_cost_events` ledger row with `surface="sms_devconsole_test"`); `TestReregisterVeloRoute` (4: anon blocked, CSRF required, `VELO_MASTER_URL` unset 400, success path calls `register_with_velo(force=True)`). Every test that exercises a probe / email / SMS / agent registration path replaces the underlying helper via `monkeypatch` so CI burns zero provider quota and never depends on a real key being present in env. The redaction layer + the three additional regression test classes were added in response to a code-review finding that the original `_http_probe` would have leaked the API key for query-param providers (Google Places, TripAdvisor) into the JSON whenever an `httpx.HTTPError` was caught — fixed with the two-layer `_collect_secrets` + `_redact` defence and pinned by the `TestProbeDoesNotLeakKey` cases so a future refactor that drops the redaction shows up as a CI failure rather than a silent secret exposure. Full suite: 386/386 (340 prior + 46 new), all green. The existing `test_admin_performance.py` (18 cases) still passes — no regression on the perf snapshot or regenerate routes that the runbook actions call into.
