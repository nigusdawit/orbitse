...
| 069 | Research Hub + Content Studio tabs (UI) | done | task/069-rce-tabs | 064,065 | Two super-admin tabs wiring all Phase-8 APIs (run research/gather/reports + generate/review/visuals/publish/capabilities/log); _rceEsc XSS-escaping; reuses CSRF fetch wrapper. Verified: node --check clean + live embedded-PG run (tabs render, all 5 read APIs return seeded data, config redaction confirmed) |
| 070 | Hardening: super-admin boundary for Datahub tools + approve endpoint | done | task/070-admin-tool-role-gating | 067 | Follow-up to 067's security review. Generalized guard → _admin_tool_superadmin_guard; gated admin_define_schema/inspect_connection/query_connection (mirror super-admin-only datahub routes) + the pending-action APPROVE endpoint (client can propose but only operator approves → executes writes). BEHAVIOR CHANGE: client role can't approve AI writes via chat. Gate 6/6 + role-boundary regression green. Per owner decision, also gated the 4 external-connections HTTP routes (list/create/delete/test) on super-admin (operator flows verified via multidb/dashboards tests); custom-sql + dashboards routes left admin-level by choice |
| 071 | Datahub AI-tool model: writes + external reads super-admin | done | task/071-datahub-write-tool-guard | 070 | Principled split: AI-driven WRITES (define_schema/save_query/create_dashboard) super-admin any connection; EXTERNAL reads (inspect/query_connection cid≠0) super-admin while app-DB cid=0 stays open to the client assistant (== admin_run_sql boundary). Replaces 070's blanket guard on the two read tools so the client "ask about your data" flow isn't regressed. Regression: datahub define/query/save_query/dashboards + publish all green |
